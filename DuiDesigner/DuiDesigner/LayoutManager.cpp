...
void CLayoutManager::Init(HWND hWnd,LPCTSTR pstrLoad)
{
	...
	if(*pstrLoad!='\0')
	{
		...
		m_Manager.SetCurResourcePath(m_strSkinDir);
...
CControlUI* CLayoutManager::NewUI(int nClass,CRect& rect,CControlUI* pParent, CLayoutManager* pLayout)
{
	...
	case classFadeButton:
		pControl = new DuiLib::CFadeButtonUI;
	...
	case classOption:
		...
		pControl->SetFloat(true);
		break;
	case classFadeOption:
		pControl = new CFadeOptionUI;
		pExtended->nClass = classFadeOption;
		pControl->SetFloat(true);
		break;
	case classCheckBox:
		...
		pControl->SetFloat(true);
		break;
	case classFadeCheckBox:
		pControl = new CFadeCheckBoxUI;
		pExtended->nClass = classFadeCheckBox;
		pControl->SetFloat(true);
		break;
	case classProgress:
	...
	case classComboBox:
		...
		break;
	case classRadio:
		pControl = new CRadioUI;
		pExtended->nClass = classRadio;
		pControl->SetFloat(true);
		break;
	case classFlash:
		pControl = new CFlashUI;
		pExtended->nClass = classFlash;
		pControl->SetFloat(true);
		break;
	case classGifAnim:
		pControl = new CGifAnimUI;
		pExtended->nClass = classGifAnim;
		pControl->SetFloat(true);
		break;
	case classList:
	...
	case classWkeBrowser:
		...
		pExtended->nClass = classWkeBrowser;
		break;
	...
	case classAnimTabLayout:
		pControl = new CAnimationTabLayoutUI;
		pExtended->nClass = classAnimTabLayout;
		break;
	case classScrollBar:
...
CControlUI* CLayoutManager::CloneControl(CControlUI* pControl)
{
	...
	case classButton:
		...
		break;
	case classFadeButton:
		pCopyControl = new CFadeButtonUI(*static_cast<CFadeButtonUI*>(pControl->GetInterface(_T("FadeButton"))));
		break;
	case classEdit:
	...
	case classRichEdit:
		...
		break;
	case classRadio:
		pCopyControl = new CRadioUI(*static_cast<CRadioUI*>(pControl->GetInterface(_T("Radio"))));
		break;
	case classOption:
		pCopyControl = new COptionUI(*static_cast<COptionUI*>(pControl->GetInterface(_T("Option"))));
		break;
	case classFadeOption:
		pCopyControl = new CFadeOptionUI(*static_cast<CFadeOptionUI*>(pControl->GetInterface(_T("FadeOption"))));
		break;
	case classCheckBox:
		pCopyControl = new CCheckBoxUI(*static_cast<CCheckBoxUI*>(pControl->GetInterface(_T("CheckBox"))));
		break;
	case classFadeCheckBox:
		pCopyControl = new CFadeCheckBoxUI(*static_cast<CFadeCheckBoxUI*>(pControl->GetInterface(_T("FadeCheckBox"))));
		break;
	case classProgress:
	...
	case classComboBox:
		...
		break;
	case classGifAnim:
		pCopyControl = new CGifAnimUI(*static_cast<CGifAnimUI*>(pControl->GetInterface(_T("GifAnim"))));
		break;
	case classActiveX:
		pCopyControl = new CActiveXUI(*static_cast<CActiveXUI*>(pControl->GetInterface(_T("ActiveX"))));
		break;
	case classFlash:
		pCopyControl = new CFlashUI(*static_cast<CFlashUI*>(pControl->GetInterface(_T("Flash"))));
		break;
	case classContainer:
	...
	case classTabLayout:
		...
		break;
	case classAnimTabLayout:
		pCopyControl = new CAnimationTabLayoutUI(*static_cast<CAnimationTabLayoutUI*>(pControl->GetInterface(_T("AnimationTabLayout"))));
		break;
	...
	case classWebBrowser:
		pCopyControl=new CWebBrowserUI(*static_cast<CWebBrowserUI*>(pControl->GetInterface(_T("WebBrowser"))));
			break;
	case classWkeBrowser:
		pCopyControl = new CWkeWebkitUI(*static_cast<CWkeWebkitUI*>(pControl->GetInterface(_T("WkeWebkit"))));
		break;
	case classDateTime:
		pCopyControl = new CDateTimeUI(*static_cast<CDateTimeUI*>(pControl->GetInterface(_T("DateTime"))));
		break;
	case classList:
	...
	case classTreeView:
		{
			pCopyControl = new CTreeViewUI(*static_cast<CTreeViewUI*>(pControl->GetInterface(_T("TreeView"))));
		}
		break;
	default:
...
void CLayoutManager::SaveControlProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	...
	//bkcolor3
	if (pControl->GetBkColor3() != 0)
	{
		DWORD dwColor = pControl->GetBkColor3();
		_stprintf_s(szBuf, _T("#%02X%02X%02X%02X"), HIBYTE(HIWORD(dwColor)), static_cast<BYTE>(GetBValue(dwColor)), static_cast<BYTE>(GetGValue(dwColor)), static_cast<BYTE>(GetRValue(dwColor)));

		pNode->SetAttribute("bkcolor3", StringConvertor::WideToUtf8(szBuf));
	}
	//bordercolor
	...
	//colorhsl
	if (pControl->IsColorHSL())
	{
		pNode->SetAttribute("colorhsl", "true");
	}
	//userdata
...
void CLayoutManager::SaveLabelProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	...
	{
		pNode->SetAttribute("enabledeffect", "true");
	}
	//rhaa
	int rhaa = pLabelUI->GetTextRenderingHintAntiAlias();
	if (rhaa >0 && rhaa <= 5)
	{
		pNode->SetAttribute("rhaa", rhaa);
	}
	//enabledshadow
	if (pLabelUI->GetEnabledShadow())
	{
		pNode->SetAttribute("enabledshadow", "true");
	}
	//transtext
	if (pLabelUI->GetTransText() != 100)
	{
		pNode->SetAttribute("transtext", pLabelUI->GetTransText());
	}
	//transshadow
	if (pLabelUI->GetTransShadow() != 100)
	{
		pNode->SetAttribute("transshadow", pLabelUI->GetTransShadow());
	}
	//transtext1
	if (pLabelUI->GetTransText1() != 100)
	{
		pNode->SetAttribute("transtext1", pLabelUI->GetTransText1());
	}
	//transshadow1
	if (pLabelUI->GetTransShadow1() != 100)
	{
		pNode->SetAttribute("transshadow1", pLabelUI->GetTransShadow1());
	}
	//gradientangle
	if (pLabelUI->GetGradientAngle() != 0)
	{
		pNode->SetAttribute("gradientangle", pLabelUI->GetGradientAngle());
	}
	//gradientlength
	if (pLabelUI->GetGradientLength() != 0)
	{
		pNode->SetAttribute("gradientlength", pLabelUI->GetGradientLength());
	}
	//enabledstroke
	if (pLabelUI->GetEnabledStroke())
	{
		pNode->SetAttribute("enabledstroke", "true");
	}
	//transstroke
	if (pLabelUI->GetTransStroke() != 255)
	{
		pNode->SetAttribute("transstroke", pLabelUI->GetTransStroke());
	}
	//textcolor1
	if (pLabelUI->GetTextColor1() != 0)
	{
		...
		_stprintf_s(szBuf, _T("#%02X%02X%02X%02X"), HIBYTE(HIWORD(dwColor)), static_cast<BYTE>(GetBValue(dwColor)), static_cast<BYTE>(GetGValue(dwColor)), static_cast<BYTE>(GetRValue(dwColor)));
		pNode->SetAttribute("textcolor1", StringConvertor::WideToUtf8(szBuf));
	}
	//textshadowcolora
	if (pLabelUI->GetTextShadowColorA() != 0)
	{
		DWORD dwColor = pLabelUI->GetTextShadowColorA();
		_stprintf_s(szBuf, _T("#%02X%02X%02X%02X"), HIBYTE(HIWORD(dwColor)), static_cast<BYTE>(GetBValue(dwColor)), static_cast<BYTE>(GetGValue(dwColor)), static_cast<BYTE>(GetRValue(dwColor)));
		pNode->SetAttribute("textshadowcolora", StringConvertor::WideToUtf8(szBuf));
	}
	//textshadowcolorb
	if (pLabelUI->GetTextShadowColorB() != 0)
	{
		DWORD dwColor = pLabelUI->GetTextShadowColorB();
		_stprintf_s(szBuf, _T("#%02X%02X%02X%02X"), HIBYTE(HIWORD(dwColor)), static_cast<BYTE>(GetBValue(dwColor)), static_cast<BYTE>(GetGValue(dwColor)), static_cast<BYTE>(GetRValue(dwColor)));
		pNode->SetAttribute("textshadowcolorb", StringConvertor::WideToUtf8(szBuf));
	}
	//strokecolor
	if (pLabelUI->GetStrokeColor() != 0)
	{
		DWORD dwColor = pLabelUI->GetStrokeColor();
		_stprintf_s(szBuf, _T("#%02X%02X%02X%02X"), HIBYTE(HIWORD(dwColor)), static_cast<BYTE>(GetBValue(dwColor)), static_cast<BYTE>(GetGValue(dwColor)), static_cast<BYTE>(GetRValue(dwColor)));
		pNode->SetAttribute("strokecolor", StringConvertor::WideToUtf8(szBuf));
	}
}
...
void CLayoutManager::SaveButtonProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	...
	if (pButtonUI->GetHotForeImage() && _tcslen(pButtonUI->GetHotForeImage()) > 0)
		pNode->SetAttribute("hotforeimage", StringConvertor::WideToUtf8(ConvertImageFileName(pButtonUI->GetHotForeImage())));
...
void CLayoutManager::SaveOptionProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	...
	if (pOptionUI->GetSelectedToolTip() && _tcslen(pOptionUI->GetSelectedToolTip()) > 0)
		pNode->SetAttribute("selectedtooltip", StringConvertor::WideToUtf8(pOptionUI->GetSelectedToolTip()));
...
void CLayoutManager::SaveSliderProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	...
	if (pSliderUI->GetChangeStep() != 0)
	{
		pNode->SetAttribute("step", pSliderUI->GetChangeStep());
	}
}
...
void CLayoutManager::SaveEditProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	...
	if (pEditUI->GetPrompt() && _tcslen(pEditUI->GetPrompt()) > 0)
		pNode->SetAttribute("prompt", StringConvertor::WideToUtf8(pEditUI->GetPrompt()));
...
void CLayoutManager::SaveListProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	...
	if (pListUI->GetScrollSelect())
		pNode->SetAttribute("scrollselect", "true");
...
void CLayoutManager::SaveComboProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	...
	if (!pComboUI->GetDropBoxAttributeList().IsEmpty())
		pNode->SetAttribute("dropbox", StringConvertor::WideToUtf8(pComboUI->GetDropBoxAttributeList()));
...
void CLayoutManager::SaveListHeaderItemProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	...
	//font
	int nFont = pListHeaderItemUI->GetFont();
	if (nFont != -1)
		pNode->SetAttribute("font", nFont);
...
{
	SaveContainerProperty(pControl, pNode);
	CTileLayoutUI* pTileLayoutUI = static_cast<CTileLayoutUI*>(pControl->GetInterface(_T("TileLayout")));
	TCHAR szBuf[MAX_PATH] = { 0 };

	if (pTileLayoutUI->GetColumns() != 0)
	{
		_stprintf_s(szBuf, _T("%d"), pTileLayoutUI->GetColumns());
		pNode->SetAttribute("columns", StringConvertor::WideToUtf8(szBuf));
	}

	SIZE szItemSize = pTileLayoutUI->GetItemSize();
	if ((szItemSize.cx != 0) || (szItemSize.cy != 0))
	{
		ZeroMemory(szBuf, sizeof(szBuf));
		_stprintf_s(szBuf, _T("%d,%d"), szItemSize.cx, szItemSize.cy);
		pNode->SetAttribute("itemsize", StringConvertor::WideToUtf8(szBuf));
	}
}
									...
									, BOOL bSaveChildren/* = TRUE*/)
{
	...
	case classTabLayout:
		...
		break;
	case classAnimTabLayout:
		SaveAnimTabLayoutProperty(pControl, pNode);
		break;
	case classHorizontalLayout:
	...
	case classWebBrowser:
		...
		break;
	case classWkeBrowser:
		SaveWkeBrowserProperty(pControl, pNode);
		break;
	default:
		break;
	}
...
void CLayoutManager::SaveItemProperty( CControlUI* pControl, TiXmlElement* pNode )
{
	...
	if (pListInfo->bShowHtml)
		pNode->SetAttribute("itemshowhtml", "true");

	if (pListInfo->bMultiExpandable)
		pNode->SetAttribute("itemtextpadding", "true");

	if (pListInfo->bAlternateBk)
		pNode->SetAttribute("itemaltbk", "true");

	if (pListInfo->bMultiSelect)
		pNode->SetAttribute("multiselect", "true");
...
void CLayoutManager::SaveAnimTabLayoutProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	SaveTabLayoutProperty(pControl, pNode);
	TCHAR szBuf[MAX_PATH] = { 0 };
	ASSERT(pControl);
	...
	pNode->SetAttribute("animation_direction", pTabLayout->IsVerticalDirection() ? "vertical" : "horizontal");
}
...
void CLayoutManager::SaveWkeBrowserProperty(CControlUI* pControl, TiXmlElement* pNode)
{
	SaveControlProperty(pControl, pNode);

	CWkeWebkitUI * pWkeBrowserUI = static_cast<CWkeWebkitUI*>(pControl->GetInterface(_T("WkeWebkit")));

	if (pWkeBrowserUI->GetUrl() && _tcslen(pWkeBrowserUI->GetUrl()) > 0)
	{
		pNode->SetAttribute("url", StringConvertor::WideToUtf8(pWkeBrowserUI->GetUrl()));
	}
}
