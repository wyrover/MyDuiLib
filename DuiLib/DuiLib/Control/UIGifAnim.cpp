...
//		InitGifImage();
	...
	void CGifAnimUI::InitGifImage()
	{
		try
		{
			...
			CDuiString sPath = m_pManager->GetCurResourcePath();
			CDuiString sZipName = m_pManager->GetCurResourceZip();
			bool bZip = sZipName.IsEmpty() ? false : true;
			if (bZip)
			{
				CDuiString pstrZip = PathUtil::CombinePath(sPath , sZipName);
				HZIP hz = NULL;
				if (CPaintManagerUI::IsZipResourceCached(pstrZip)) hz = (HZIP)CPaintManagerUI::GetCachedZipResource(pstrZip);
				else hz = OpenZip((void*)pstrZip.GetData(), 0, 2);
				if (hz == NULL) return;
				ZIPENTRY ze;
				int i;
			...
			{
				CDuiString pstrZip = PathUtil::CombinePath(sPath, sImageName);
				m_pGifImage = Image::FromFile(CStringW((LPCTSTR)pstrZip));
			}
		}
		catch (...)
		{
