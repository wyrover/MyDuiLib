...
//CTextServiceHelper
//////////////////////////////////////////////////////////////////////
class CTextServiceHelper : public Singleton<CTextServiceHelper>
{
public:
	typedef HRESULT(_stdcall *PCreateTextServices)(IUnknown *punkOuter, ITextHost *pITextHost, IUnknown **ppUnk);
	CTextServiceHelper()
	{
		hModuleRiched20 = ::LoadLibrary(_T("Riched20.dll"));	// 加载RichEdit控件DLL
		if (hModuleRiched20)
			pCreateTextServices = (PCreateTextServices)GetProcAddress(hModuleRiched20, "CreateTextServices");
	}
	~CTextServiceHelper()
	{
		if (hModuleRiched20)
	...
	{
		if (!pCreateTextServices) return E_NOTIMPL;
		return pCreateTextServices(punkOuter, pITextHost, ppUnk);
	}

protected:
	HMODULE    	hModuleRiched20;	//richedit module
...
//  member functions
//////////////////////////////////////////////////////////////////////
void CImageDataObjectHandler::InsertBitmap(LPCTSTR lpValue)
{
	HINSTANCE hInst = ::GetModuleHandle(NULL);
	HBITMAP hBitmap = LoadBitmap(hInst,lpValue);
	InsertBitmap(hBitmap);
}
void CImageDataObjectHandler::InsertBitmap(HBITMAP hBitmap)
{
...
//	CImageDataObjectHandler *pods = this;
...
void CImageDataObjectHandler::SetBitmap(HBITMAP hBitmap)
{
	ASSERT(hBitmap);
...
IOleObject * CImageDataObjectHandler::GetOleObject(IOleClientSite *pOleClientSite, IStorage *pStorage)
{
...
public:
	...
	{ 
		...
		pserv->TxSendMessage(EM_GETOLEINTERFACE, 0, (LPARAM) (LPVOID*) &pRichEditOle,&lResult);
...
CTxtWinHost::CTxtWinHost() : m_re(NULL), pcb(NULL)
{
...
CRichEditUI::CRichEditUI() : m_pTwh(NULL), m_bVScrollBarFixing(false), m_bWantTab(true), m_bWantReturn(true), 
	...
	m_iLimitText(cInitTextMax), m_lTwhStyle(ES_MULTILINE), m_bInited(false), m_pImageObjectCB(NULL)
{
...
CRichEditUI::~CRichEditUI()
{
	...
	if (m_pImageObjectCB)
	{
		m_pImageObjectCB->Release();
		delete m_pImageObjectCB;
		m_pImageObjectCB = NULL;
	}
}
...
void CRichEditUI::DoInit()
{
	...
	CImageDataObjectHandler* pObject = new CImageDataObjectHandler(this);
...
{ 
	if (NULL == m_pTwh)
		return NULL;
	return m_pTwh->GetRichEditOle();
}

bool CRichEditUI::SetImageObject(IImageDataObjectCallback *pImageObject)
{
	if (m_pImageObjectCB)
	{
		m_pImageObjectCB->Release();
	...
	m_pImageObjectCB = pImageObject;
	return true;
}

IImageDataObjectCallback* CRichEditUI::GetImageObject()
{
	return m_pImageObjectCB;
}
...
{
	if (m_pImageObjectCB)
		m_pImageObjectCB->InsertBitmap(hBitmap);
}

void CRichEditUI::InsertBitmap(LPCTSTR lpValue)
{
	if (m_pImageObjectCB)
		m_pImageObjectCB->InsertBitmap(lpValue);
}
