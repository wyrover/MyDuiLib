...
CControlUI* IDialogBuilderCustomHandle::CreateFromXML(STRINGorID xml, LPCTSTR type /* = NULL */, CPaintManagerUI* pManager /* = NULL */, CControlUI* pParent /* = NULL */)
{
	CMarkup m_xml;
	...
	if (HIWORD(xml.m_lpstr) != NULL) {
		if (*(xml.m_lpstr) == _T('<')) {
			...
			if (pManager)
			{
				CDuiString sPath = pManager->GetCurResourcePath();
				CDuiString sZip = pManager->GetCurResourceZip();
				if (sZip.IsEmpty())
				{
					if (!m_xml.LoadFromFileDirectly(sPath, xml.m_lpstr)) return NULL;
				}
				else
				{
					CDuiString szFullZipName = PathUtil::CombinePath(sPath, sZip);
					if (!m_xml.LoadFromZipFile(szFullZipName.GetData(), xml.m_lpstr)) return NULL;
				}
			}
			return NULL;
		}
		...
		HRSRC hResource = ::FindResource(hCurInst, xml.m_lpstr, type);
		...
		if (!m_xml.LoadFromMem((BYTE*)::LockResource(hGlobal), ::SizeofResource(hCurInst, hResource))) return NULL;
		::FreeResource(hResource);
	}


	return CreateFromXML(&m_xml, type, pManager, pParent);
}

CControlUI* IDialogBuilderCustomHandle::CreateFromXML(CMarkup* pXML, LPCTSTR type/* = NULL*/, CPaintManagerUI* pManager /* = NULL */, CControlUI* pParent /* = NULL */)
{
	if (!pXML) return NULL;
	CMarkupNode root = pXML->GetRoot(); //tag = Window
	...
	return _Parse(&root, type, pParent, pManager);
}

CControlUI* IDialogBuilderCustomHandle::_Parse(CMarkupNode* pRoot, LPCTSTR strtype /*= NULL*/, CControlUI* pParent /* = NULL */, CPaintManagerUI* pManager /* = NULL */)
{
	...
	for (CMarkupNode node = pRoot->GetChild(); node.IsValid(); node = node.GetSibling()) {
		...
		if (_tcscmp(pstrClass, _T("Include")) == 0) {
			...
			for (int i = 0; i < count; i++) {
				if (strtype != NULL) { // 使用资源dll，从资源中读取
					WORD id = (WORD)_tcstol(szValue, &pstr, 10);
					pControl = CreateFromXML((UINT)id, strtype, pManager, pParent);
				}
				else {
					pControl = CreateFromXML((LPCTSTR)szValue, (UINT)0, pManager, pParent);
				}
			}
			continue;
		}
		//树控件XML解析
			...
			//检索子节点及附加控件
			if (node.HasChildren()){
				CControlUI* pSubControl = _Parse(&node, strtype, pNode, pManager);
			...
			TCHAR szDuiClassName[200] = { 0 };
			_stprintf(szDuiClassName, _T("C%sUI"), pstrClass);
			
			pControl = CreateDuiInstance<CControlUI*>(szDuiClassName);
		...
		// Add children
		if (node.HasChildren()) {
			_Parse(&node, strtype, pControl, pManager);
		}
		// Attach to parent
