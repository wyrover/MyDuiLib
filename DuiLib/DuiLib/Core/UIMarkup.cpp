...
int CMarkupNode::GetAttributeInt(LPCTSTR pstrName)
{
	LPCTSTR pstrValue = GetAttributeValue(pstrName);
	return _tcstol(pstrValue, NULL, 10);
}
bool CMarkupNode::GetAttributeBool(LPCTSTR pstrName)
{
	LPCTSTR pstrValue = GetAttributeValue(pstrName);
	return _tcscmp(pstrValue, _T("true")) == 0;
}
float CMarkupNode::GetAttributeFloat(LPCTSTR pstrName)
{
	LPCTSTR pstrValue = GetAttributeValue(pstrName);
	LPTSTR pstr = NULL;
	return _tcstof(pstrValue, &pstr);
}
...
bool CMarkup::LoadFromFileDirectly(LPCTSTR pstrPath, LPCTSTR pstrFilename, int encoding)
{
	Release();
	CDuiString sFile = PathUtil::CombinePath(pstrPath, pstrFilename);
...
bool CMarkup::LoadFromZipFile(LPCTSTR pstrZip, LPCTSTR pstrFilename, int encoding)
{
	Release();
	CDuiString sFile = pstrZip;

	HZIP hz = NULL;
	if (CPaintManagerUI::IsZipResourceCached(pstrZip)) hz = (HZIP)CPaintManagerUI::GetCachedZipResource(pstrZip);
	...
	if (res != 0x00000000 && res != 0x00000600) {
		delete[] pByte;
		if (!CPaintManagerUI::IsZipResourceCached(pstrZip)) CloseZip(hz);
