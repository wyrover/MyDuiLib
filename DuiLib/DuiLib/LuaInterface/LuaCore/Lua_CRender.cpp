...
namespace DuiLib
{
	LUA_METHOD_IMPL(CRenderClip, isnil)
	{
		CRenderClip* self;
	...
	LUA_METHOD_IMPL(CRenderClip, _gc)
	{	
		CRenderClip* self;
		self = static_cast<CRenderClip*>(LuaStatic::CheckUserData(l, 1));
	...
	LUA_METHOD_IMPL(CRenderClip, _eq)
	{
		try
		{
			CRenderClip* left;
	...
	LUA_METHOD_IMPL(CRenderClip, _tostring)
	{
		try
		{
			CRenderClip* self;
	...
	LUA_METHOD_IMPL(CRenderClip, Delete)
	{
		CRenderClip* self;
		self = static_cast<CRenderClip*>(LuaStatic::CheckUserData(l, 1));
		LuaStatic::RemoveObjectInLua(l);
	...
	LUA_METHOD_IMPL(CRenderClip, New)
	{
		CRenderClip  *self = new CRenderClip();
		LuaStatic::AddObject2Lua(l, self, "CRenderClip");
		return 1;
	}
	LUA_METHOD_IMPL(CRenderClip, GetClassName)
	{
		lua_pushstring(l, "RenderClip");
		return 1;
	}
	LUA_METHOD_IMPL(CRenderClip, GenerateClip)
	{
	...
	LUA_METHOD_IMPL(CRenderClip, GenerateRoundClip)
	{
		HDC hDC = static_cast<HDC>(LuaStatic::CheckUserData(l, 1));
		RECT rc = LuaStatic::ReadRectFromLua(l,2);
	...
	LUA_METHOD_IMPL(CRenderClip, UseOldClipBegin)
	{
		HDC hDC = static_cast<HDC>(LuaStatic::CheckUserData(l, 1));
	...
	LUA_METHOD_IMPL(CRenderClip, UseOldClipEnd)
	{
		HDC hDC = static_cast<HDC>(LuaStatic::CheckUserData(l, 1));
		CRenderClip *self = static_cast<CRenderClip*>(LuaStatic::CheckUserData(l, 2));
	...
	LUA_METHOD_IMPL(CRenderClip, setHDC)
	{
		CRenderClip *self = static_cast<CRenderClip*>(LuaStatic::CheckUserData(l, 1));
	...
	LUA_METHOD_IMPL(CRenderClip, setHRGN)
	{
		CRenderClip *self = static_cast<CRenderClip*>(LuaStatic::CheckUserData(l, 1));
	...
	LUA_METHOD_IMPL(CRenderClip, setOldHRGN)
	{
		CRenderClip *self = static_cast<CRenderClip*>(LuaStatic::CheckUserData(l, 1));
	...
	LUA_CLASS_REG_IMPL(CRenderClip)
	{
		lua_newtable(l); // t,
...
namespace DuiLib
{
	LUA_METHOD_IMPL(CRenderEngine, AdjustColor)
	{
		try
		{
	...
	LUA_METHOD_IMPL(CRenderEngine, LoadBitmap)
	{
		try
		{
			...
			{
				pManager = static_cast<CPaintManagerUI*>(LuaStatic::CheckUserData(l, 1));
			}
			...
			if (lua_isstring(l, 2))
			{
				...
				bitmap = STRINGorID(lua_tointeger(l,2));
			LPCTSTR type = NULL;
			if (lua_isstring(l,3))
			{
				CDuiString pstrText;
			...
			HBITMAP hBitmap = CRenderEngine::LoadBitmap(pManager,bitmap, type, mask);
			if (hBitmap)
	...
	LUA_METHOD_IMPL(CRenderEngine, LoadImage)
	{
		try
		{
			CPaintManagerUI* pManager = NULL;
			...
			if(lua_isnumber(l,4))
				mask = (DWORD)lua_tonumber(l,4);	
			TImageInfo* pImageInfo = CRenderEngine::LoadImage(pManager,bitmap, type, mask);
			if (pImageInfo)
	...
	LUA_METHOD_IMPL(CRenderEngine, FreeImage)
	{
		try
		{
	...
	LUA_METHOD_IMPL(CRenderEngine, DrawImage)
	{
		try
		{
			HDC hDC = static_cast<HDC>(LuaStatic::CheckUserData(l, 1));
	...
	LUA_METHOD_IMPL(CRenderEngine, DrawImageString)
	{
		try
		{
			HDC hDC = static_cast<HDC>(LuaStatic::CheckUserData(l, 1));
	...
	LUA_METHOD_IMPL(CRenderEngine, DrawColor)
	{
		try
		{
			HDC hDC = static_cast<HDC>(LuaStatic::CheckUserData(l, 1));
	...
	LUA_METHOD_IMPL(CRenderEngine, DrawGradient)
	{
		try
		{
			HDC hDC = static_cast<HDC>(LuaStatic::CheckUserData(l, 1));
	...
	LUA_METHOD_IMPL(CRenderEngine, DrawLine)
	{
		try
		{
			HDC hDC = static_cast<HDC>(LuaStatic::CheckUserData(l, 1));
	...
	LUA_METHOD_IMPL(CRenderEngine, DrawRect)
	{
		try
		{
			HDC hDC = static_cast<HDC>(LuaStatic::CheckUserData(l, 1));
	...
	LUA_METHOD_IMPL(CRenderEngine, DrawRoundRect)
	{
		try
		{
			HDC hDC = static_cast<HDC>(LuaStatic::CheckUserData(l, 1));
	...
	LUA_METHOD_IMPL(CRenderEngine, DrawText)
	{
		try
		{
			HDC hDC = static_cast<HDC>(LuaStatic::CheckUserData(l, 1));
	...
	LUA_METHOD_IMPL(CRenderEngine, DrawHtmlText)
	{
		try
		{
			HDC hDC = static_cast<HDC>(LuaStatic::CheckUserData(l, 1));
	...
	LUA_METHOD_IMPL(CRenderEngine, GenerateBitmap)
	{
		try
		{
	...
	LUA_METHOD_IMPL(CRenderEngine, GetTextSize)
	{
		try
		{
			HDC hDC = static_cast<HDC>(LuaStatic::CheckUserData(l, 1));
	...
	LUA_CLASS_REG_IMPL(CRenderEngine)
	{
		lua_newtable(l); // t,
