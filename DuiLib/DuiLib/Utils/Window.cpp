...
namespace DuiLib 
{
	DUI_IMPLAMENT_WM_MSG_MAP_BEGIN(CWin)
	...
	void CWin::OnFinalMessage( HWND hWnd )
	{
		...
		_bValid = FALSE;
		UnRegisterSkin();	
		DuiLogInfo(_T("CWin::OnFinalMessage..."));
	}
...
#pragma region RESOURCE_TYPE
	LPBYTE CWin::m_lpResourceZIPBuffer = NULL;
	UILIB_RESOURCETYPE CWin::GetResourceType() const
	{
		return UILIB_FILE;
	}
	CDuiString CWin::GetZIPFileName() const
	{
		return _T("");
	}
	LPCTSTR CWin::GetResourceID() const
	{
		return _T("");
	}
#pragma endregion RESOURCE_TYPE
	CControlUI* CWin::CreateRoot(STRINGorID xml, LPCTSTR type /* = NULL */, IDialogBuilderCallback* pCallback /* = NULL */, CControlUI* pParent /* = NULL */)
	{
		...
		_paintManager->SetCurResourcePath(strResourcePath.GetData());
