...
namespace DuiLib
{
	...
	inline int CUIAnim::TGIFControlExt::GetPackedValue(enum ControlExtValues Value)
	{
		...
		switch(Value)
		{
		case CUIAnim::ControlExtValues::GCX_PACKED_DISPOSAL:
			nRet = (nRet & 28) >> 2;
			break;

		case CUIAnim::ControlExtValues::GCX_PACKED_USERINPUT:
			nRet = (nRet & 2) >> 1;
			break;

		case CUIAnim::ControlExtValues::GCX_PACKED_TRANSPCOLOR:
			nRet &= 1;
			break;
		};
	...
	inline int CUIAnim::TGIFLSDescriptor::GetPackedValue(enum LSDPackedValues Value)
	{
		...
		switch(Value)
		{
		case CUIAnim::LSDPackedValues::LSD_PACKED_GLOBALCT:
			nRet = nRet >> 7;
			break;

		case CUIAnim::LSDPackedValues::LSD_PACKED_CRESOLUTION:
			nRet = ((nRet & 0x70) >> 4) + 1;
			break;

		case CUIAnim::LSDPackedValues::LSD_PACKED_SORT:
			nRet = (nRet & 8) >> 3;
			break;

		case CUIAnim::LSDPackedValues::LSD_PACKED_GLOBALCTSIZE:
			nRet &= 7;
			break;
		};
	...
	inline int CUIAnim::TGIFImageDescriptor::GetPackedValue(enum IDPackedValues Value)
	{
		...
		switch(Value)
		{
		case CUIAnim::IDPackedValues::ID_PACKED_LOCALCT:
			nRet >>= 7;
			break;

		case CUIAnim::IDPackedValues::ID_PACKED_INTERLACE:
			nRet = ((nRet & 0x40) >> 6);
			break;

		case CUIAnim::IDPackedValues::ID_PACKED_SORT:
			nRet = (nRet & 0x20) >> 5;
			break;

		case CUIAnim::IDPackedValues::ID_PACKED_LOCALCTSIZE:
			nRet &= 7;
			break;
		};
	...
	const TImageInfo* CUIAnim::LoadGIF(CPaintManagerUI* pManager, LPCTSTR bitmap, LPCTSTR type/* = NULL*/, DWORD mask/* = 0*/)
	{
		LPBYTE pData = NULL;
		DWORD dwSize = 0;
		...
		{
			CDuiString sFile = pManager->GetCurResourcePath();
			if (pManager->GetCurResourceZip().IsEmpty()) {
				sFile = PathUtil::CombinePath(sFile, bitmap);
			...
			{
				sFile = PathUtil::CombinePath(sFile, pManager->GetCurResourceZip());
				HZIP hz = NULL;
				if (CPaintManagerUI::IsZipResourceCached(sFile)) hz = (HZIP)CPaintManagerUI::GetCachedZipResource(sFile);
				...
				{
					if (!CPaintManagerUI::IsZipResourceCached(sFile))
						CloseZip(hz);
					return NULL;
				}
				dwSize = ze.unc_size;
		...
		bool bHole, bool bTiledX, bool bTiledY)
	{
		...
		if ( !(data=GetAniImage((LPCTSTR)sImageName)) && !(data=pManager->GetImage((LPCTSTR)sImageName)) )
		{
			// new Image
			// try GIF first
			data = AddAniImage(pManager->GetPaintDC(), pManager, (LPCTSTR)sImageName, (LPCTSTR)sImageResType, dwMask);
			if (!data)
			{
				// may be not GIF
	...
	const TImageInfo* CUIAnimHelper::AddAniImage(HDC hDC, CPaintManagerUI* pManager, LPCTSTR bitmap, LPCTSTR type/* = NULL*/, DWORD mask/* = 0*/)
	{
		CUIAnim* pAniImage = new CUIAnim(this);
		if (!pAniImage->LoadGIF(pManager,bitmap, type, mask))
		{
			delete pAniImage;
			return NULL;
		}
