...
void DuiLib::CWebBrowserUI::DoSize()
{
	UpdateScrollBar();
}

void DuiLib::CWebBrowserUI::SetScrollPos(SIZE szPos)
{
	...
	case DISPID_DOCUMENTCOMPLETE:
		{
			...
			if (pBody)
				pBody->put_scroll(_T("no"));// 去滚动条
			IHTMLStyle* pStyle = GetHTMLStyle();
			if (pStyle)
				pStyle->put_border(_T("none"));//去除边网页边框
		}
		break;
...
IHTMLElement* DuiLib::CWebBrowserUI::GetHTMLElement()
{
...
IHTMLBodyElement* DuiLib::CWebBrowserUI::GetHTMLBodyElement()
{
...
IHTMLStyle* DuiLib::CWebBrowserUI::GetHTMLStyle()
{
