...
CStdStringPtrMap CPaintManagerUI::m_aCurZipCached;
////////////////////////////////////////////////////////////////////////////////////////////
...
const TImageInfo* CPaintManagerUI::AddImage(LPCTSTR bitmap, LPCTSTR type, DWORD mask)
{
    ...
    if( type != NULL ) {
		...
		{
			data = CRenderEngine::LoadImage(this,bitmap, type, mask);
		}
    }
    else {
...
CDuiString CPaintManagerUI::GetCurResourcePath()
{
	return m_pStrCurResourcePath;
}
void CPaintManagerUI::SetCurResourcePath(LPCTSTR lpstrPath)
{
	m_pStrCurResourcePath = lpstrPath;
}

CDuiString CPaintManagerUI::GetCurResourceZip()
{
	return m_pStrCurResourceZip;
}
void CPaintManagerUI::SetCurResourceZip(LPCTSTR lpstrZip, bool bCached)
{
	m_pStrCurResourceZip = lpstrZip;
	if (bCached)
	{
		CDuiString sPath = this->GetCurResourcePath();
		CDuiString sZipFullPath = PathUtil::CombinePath(sPath, lpstrZip);
		AddZipResourceCached(sZipFullPath.GetData());
	}
}

bool CPaintManagerUI::IsZipResourceCached(LPCTSTR lpstrZip)
{
	if (!lpstrZip) return false;
	if (m_aCurZipCached.Find(lpstrZip) == NULL)
		return false;
	return true;
}

void CPaintManagerUI::AddZipResourceCached(LPCTSTR lpstrZip)
{
	if (IsZipResourceCached(lpstrZip))
		return;

	if (lpstrZip) 
	{
		CDuiString sFile = lpstrZip;
		HANDLE hResourceZip = (HANDLE)OpenZip((void*)sFile.GetData(), 0, 2);

		m_aCurZipCached.Set(lpstrZip, (LPVOID)hResourceZip);
	}
}
HANDLE CPaintManagerUI::GetCachedZipResource(LPCTSTR lpstrZip)
{
	if (!IsZipResourceCached(lpstrZip))
		return NULL;

	HANDLE hResourceZip = static_cast<HANDLE>(m_aCurZipCached.Find(lpstrZip));
	return hResourceZip;
}
