	...
	do 
	{
		if( type == NULL ) {
			if (pManager->GetCurResourceZip().IsEmpty()) {
				CDuiString sFile = PathUtil::CombinePath(pManager->GetCurResourcePath().GetData(), bitmap.m_lpstr);
				...
				CDuiString sFile = PathUtil::CombinePath(pManager->GetCurResourcePath().GetData(), pManager->GetCurResourceZip().GetData());
				HZIP hz = NULL;
				...
				if( res != 0x00000000 && res != 0x00000600) {
					delete[] pData;
					pData = NULL;
					if (!CPaintManagerUI::IsZipResourceCached(sFile)) CloseZip(hz);
					break;
				}
				if (!CPaintManagerUI::IsZipResourceCached(sFile)) CloseZip(hz);
			}
			...
			HINSTANCE hInstRes = CPaintManagerUI::GetResourceDll();
...
////////////////////////////////////////////
HBITMAP CRenderEngine::LoadBitmap(CPaintManagerUI* pManager, STRINGorID bitmap, LPCTSTR type /* = NULL */, DWORD mask /* = 0 */)
{
	LPBYTE pData = NULL;
	DWORD dwSize = 0;
