...
namespace DuiLib
{
	void CGifImage_Str(LPCTSTR buffer, dui_WString& sOut)
	{
	...
	BOOL CGifImage::LoadFromFile(LPCTSTR pszFileName)
	{
		...
		dui_WString sFileName;
		CGifImage_Str(pszFileName, sFileName);
		m_pImage = new Gdiplus::Image(sFileName.c_str());
		if (NULL == m_pImage)
			return FALSE;
	...
	BOOL CGifImage::SaveAsFile(LPCTSTR pszFileName)
	{
		...
		dui_WString sFileName;
		CGifImage_Str(lpExtension, sFileName);
		CLSID clsid = GetEncoderClsidByExtension(sFileName.c_str());
		if (CLSID_NULL == clsid)
			return FALSE;

		dui_WString sFileName2;
		CGifImage_Str(pszFileName, sFileName2);
		Gdiplus::Status status = m_pImage->Save(sFileName2.c_str(), &clsid, NULL);
