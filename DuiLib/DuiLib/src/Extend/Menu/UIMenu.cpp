...
CMenuWnd::CMenuWnd(LPCTSTR path, LPCTSTR zip,HWND hParent) :
m_hParent(hParent),
m_pOwner(NULL),
m_pLayout(),
m_xml(_T("")),
...
_data(0)
{
	m_pm.SetCurResourcePath(path);
	m_pm.SetCurResourceZip(zip);
}

CMenuWnd::CMenuWnd(CPaintManagerUI* ppm, HWND hParent) :
m_hParent(hParent),
m_pOwner(NULL),
m_pLayout(),
m_xml(_T("")),
...
_data(0)
{
	m_pm.SetCurResourcePath(ppm->GetCurResourcePath());
	m_pm.SetCurResourceZip(ppm->GetCurResourceZip());
}
...
void CMenuElementUI::CreateMenuWnd()
{
	...
	m_pWindow = new CMenuWnd(m_pManager->GetCurResourcePath(),m_pManager->GetCurResourceZip(),m_pManager->GetPaintWindow());
	ASSERT(m_pWindow);
