...
namespace DuiLib{
	...
	DWORD WINAPI CheckThreadFun(LPVOID lpParam)
	{
		...
		if (NULL == pWebView)
			return 1;
		IWkeWebkitLoadCallback* pCallback = pWebkitUI->GetLoadCallback();
		if (NULL == pCallback)
			return 1;
		bool bOver = false;
	...
	int CWkeWebkitUI::m_nTimerID = 100;
	...
	{
		return CWkeWebkitUI::GetClassName();
	}
	...
	void CWkeWebkitUI::DoInit()
	{
		if (!m_strUrl.IsEmpty())
			Navigate(m_strUrl);
		m_pManager->SetTimer(this, m_nTimerID, 100);
	}
	...
	void CWkeWebkitUI::SetLoadCallback(IWkeWebkitLoadCallback* pCallback)
	{
		m_pLoadCallback = pCallback;
	}

	IWkeWebkitLoadCallback* CWkeWebkitUI::GetLoadCallback()
	{
