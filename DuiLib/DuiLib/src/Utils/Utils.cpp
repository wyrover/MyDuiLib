...
namespace DuiLib
{
	...
	void CDuiString::AssignString(const char* pstr, int cchMax)
	{
		if( pstr == NULL ) pstr = ("");		
		...
		size_t cclen = (size_t)cchMax;
		if (cclen > 0 && cclen < len) len = cclen;
		if (len == 0)
		{
			(*this) += _T("");
			return;
...
#    if defined(_WIN32) || defined(__WIN32__) || defined(WIN32) || defined(_WIN64) || defined(WIN64) || defined(__WIN64__)
		int size = MultiByteToWideChar(CP_ACP, 0, sub_str.c_str(), -1, NULL, 0) - 1;
#	else
		size_t size = mbstowcs(NULL, sub_str.c_str(), 0);
#	endif

		std::wstring wstr(size, wchar_t(0));
#    if defined(_WIN32) || defined(__WIN32__) || defined(WIN32) || defined(_WIN64) || defined(WIN64) || defined(__WIN64__)
		MultiByteToWideChar(CP_ACP, 0, sub_str.c_str(), sub_str.length(), const_cast<wchar_t*>(wstr.c_str()), size);
#	else
		mbstowcs(const_cast<wchar_t*>(wstr.c_str()), sub_str.c_str(), size);
#	endif	
	...
	// 函数说明: 
	//************************************
	std::wstring CDuiString::GetWString() const
	{
#ifdef _UNICODE
		return GetData();
#else
	...
	// 函数说明: 
	//************************************
	std::string CDuiString::GetAString() const
	{
#ifdef _UNICODE
		if(!IsEmpty())
		{
...
#ifdef _UNICODE
	std::wstring CDuiString::GetString() const
	{
		return GetWString();
	}
#else
	std::string CDuiString::GetString() const
	{
		return GetAString();
	}
#endif
