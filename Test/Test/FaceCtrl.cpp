	...
	if( event.Type == UIEVENT_MOUSEMOVE )
	{
		...
		if (nIndex != m_nHoverIndex)
		{
			...
			if (m_nHoverIndex != -1)
			{
				...
				if (lpFaceInfo != NULL)
					SetToolTip(lpFaceInfo->m_strTip);
...
BOOL CFaceCtrl::LoadFace(int nPageIndex)
{
	if (NULL == m_lpFaceList)
		return FALSE;
	...
	for (int i = nStart; i < nEnd; i++)
	{
		...
		if (lpFaceInfo != NULL)
		{
			...
			if (lpImage != NULL)
			{
				CDuiString sFileFloder = GetFaceFloder();
				BOOL bRet = lpImage->LoadFromFile((sFileFloder+lpFaceInfo->m_strFileName));
				if (bRet)
...
CDuiString& CFaceCtrl::GetFaceFloder()
{
	static CDuiString sFileFloder = CPaintManagerUI::GetInstancePath() + _T("richedit_test\\");
	return sFileFloder;
}

void CFaceCtrl::DestroyFace()
{
	for (int i = 0; i < (int)m_arrImage.size(); i++)
	{
		CGifImage * lpImage = m_arrImage[i];
