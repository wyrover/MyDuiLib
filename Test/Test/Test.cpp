...
int RunRichEditTest(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	CDuiString strFileName = PathUtil::GetAppPath();
	
	strFileName.Append(_T("ImageOleCtrl.dll"));
	BOOL bRet = DuiSystem::Instance()->DllRegisterServer((strFileName));	// 注册COM组件
	if (!bRet)
	{
	...
	CPaintManagerUI::SetInstance(hInstance);
	//CPaintManagerUI::SetResourcePath(CPaintManagerUI::GetInstancePath() + _T("richedit_test/skin"));
	...
	DuiSystem::Instance()->DllUnregisterServer((strFileName));
