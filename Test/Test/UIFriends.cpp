...
namespace DuiLib
{
	const int kFriendListItemNormalHeight = 30;
	const int kFriendListItemSelectedHeight = 54;
...
CFriendsUI::CFriendsUI(CPaintManagerUI& paint_manager)
	...
	, delay_number_(0)
	, delay_left_(0)
	, level_expand_image_(_T("<i icon_expand.png>"))
	, level_collapse_image_(_T("<i icon_collapse.png>"))
...
Node* CFriendsUI::AddNode(const FriendListItemInfo& item, Node* parent)
{
	...
	if(! m_dlgBuilder.GetMarkup()->IsValid() ) {
        pListElement = static_cast<CListContainerElementUI*>(m_dlgBuilder.Create(_T("FriendItem_d.xml"), (UINT)0, NULL, &paint_manager_));
    }
    else {
	...
	if (!item.folder && !item.description.IsEmpty())
	{
		...
		{
#if defined(UNDER_WINCE)
			_stprintf(szBuf, _T("<c #808080>%s</c>"), item.description.GetData());
#else
			_stprintf_s(szBuf, MAX_PATH - 1, _T("<c #808080>%s</c>"), item.description.GetData());
#endif
	...
	//* pOperatorPannel = static_cast<CContainerUI*>(paint_manager_.FindSubControlByName(pListElement, kOperatorPannelControlName));
