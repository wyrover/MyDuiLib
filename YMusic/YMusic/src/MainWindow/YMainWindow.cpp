...
{
	CControlUI* pSender = msg.pSender;
	CMenuWnd* pMenu = new CMenuWnd(theMainWin->GetPaintMgr(),GetHWND());
	POINT pt;
	GetCursorPos(&pt);
...
{
	CMenuWnd* pMenu = new CMenuWnd(GetPaintMgr(), GetHWND());
	POINT pt;
	GetCursorPos(&pt);
...
{
	CMenuWnd* pModeMenu = new CMenuWnd(GetPaintMgr(), GetHWND());
	POINT pt;
	GetCursorPos(&pt);
...
LRESULT YMainWindow::OnHotKey(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	for (int i=0;i<sizeof(hk)/sizeof(hk[0]);++i)
	{
		...
		wModifiers = DuiSystem::Instance()->HKF2SDK( HIBYTE(LOWORD(hk[i].dwKey)));
...
void YMainWindow::UpdatePlayList()
{
	...
	for (PlayListMgr::playListSongMapIterT it = listMap.begin(),iend = listMap.end();it != iend;++it)
	{
		...
		theDbMgr->GetUserDB()->GetPlayList(spListInfo,it->first);
 		...
 		YPlayListItemUI* pListItem = new YPlayListItemUI(GetPaintMgr());
 		ASSERT(pListItem);
...
void YMainWindow::UpdateSongList(int nPlayListId)
{
	...
	for (PlayListMgr::songsArrayIterT it=songList.begin(),iend = songList.end();it!=iend;++it)
	{
		...
		YSongListItemUI* spSongListItem = new YSongListItemUI(GetPaintMgr());
...
//			unsigned int u_listId = (unsigned int)(_ui._pSongList->GetTag());
			...
			YSongListItemUI* songItem = new YSongListItemUI(GetPaintMgr());
			ASSERT(songItem);
