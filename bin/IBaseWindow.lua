...
def.method().TouchMsgTable = function(self)
	local msgt = 
	{
		...
		GetSkinFolder = self:tryget("GetSkinFolder"),
		GetZIPFileName = self:tryget("GetZIPFileName"),
		GetResourceID = self:tryget("GetResourceID"),
		GetResourceType = self:tryget("GetResourceType"),
	}
