...
#if USE(ZIP_SKIN)
	CStdString tstrSkin = GetSkinFile();
...
#else
	CStdString tstrSkin = paint_manager_.GetResourcePath();
	tstrSkin += GetSkinFile();
#endif

	CControlUI* pRoot = builder.Create(GetSkinFile().GetData(), (UINT)0, this, &paint_manager_);
