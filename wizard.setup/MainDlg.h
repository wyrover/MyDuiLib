...
public:
	...
	LRESULT OnInstall(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if(GetFileAttributes(_T("DuiLibWizard"))==INVALID_FILE_ATTRIBUTES)
		{
			MessageBox(_T("当前目录下没有找到DuiLib的向导数据"),_T("错误"),MB_OK|MB_ICONSTOP);
			return 0;
		}
		...
		_tcscat(szDuiEngineDir,_T("\\DuiLib"));
		if(GetFileAttributes(szDuiEngineDir)==INVALID_FILE_ATTRIBUTES)
		{
			MessageBox(_T("当前目录下没有找到DuiLib的源代码"),_T("错误"),MB_OK|MB_ICONSTOP);
			return 0;
		}
		...
		{
			reg.SetStringValue(_T("DUILIBPATH"),szDuiEngineDir);
			reg.Close();
		...
		for(int i=0;i<vslist.GetItemCount();i++)
		{
			...
			if(bOK)
			{
				...
				_tcscpy(szFrom,_T("DuiLibWizard"));
			...
			if(bOK)
			{
				...
				CString strTo=pCfg->strVsDir+pCfg->strDataTarget+_T("\\DuiLibWizard\\Scripts\\2052");
				_tcscpy(szTo,strTo);
				bOK= 0==SHFileOperation(&shfo);
			}
			CString strMsg;
	...
	LRESULT OnUninstall(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		for(int i=0;i<vslist.GetItemCount();i++)
		{
			...
			{
				TCHAR szBuf[1000]={0};
				CString strFrom=pCfg->strVsDir+pCfg->strDataTarget+_T("\\DuiLibWizard");
			...
			if(bOK)
			{
				CString strSource=pCfg->strVsDir+pCfg->strEntryTarget+_T("\\DuiLibWizard.ico");
				DeleteFile(strSource);
				strSource=pCfg->strVsDir+pCfg->strEntryTarget+_T("\\DuiLibWizard.vsdir");
				bOK = DeleteFile(strSource);
				strSource=pCfg->strVsDir+pCfg->strEntryTarget+_T("\\DuiLibWizard.vsz");
				bOK = DeleteFile(strSource);
			}
			
			CString strMsg;
			strMsg.Format(_T("从%s中卸载DuiLib向导%s"),pCfg->strName,bOK?_T("成功"):_T("失败"));
			::SendMessage(GetDlgItem(IDC_LOG),LB_ADDSTRING,0,(LPARAM)(LPCTSTR)strMsg);
