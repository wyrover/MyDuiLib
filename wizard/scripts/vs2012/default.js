...
function AddConfig(proj, strProjectName)
{
	try
	{
		...
		var isDllDuiLib = wizard.FindSymbol('CHECKBOX_DUILIB_DLL');//从用户界面中获取DUILIB的链接方式
		...
		CLTool.AdditionalIncludeDirectories = '"$(DUILIBPATH)\\DUILIB\\include";';//%(AdditionalIncludeDirectories)';
		if(isDllDuiLib)
		...
		LinkTool.GenerateDebugInformation = true;
		LinkTool.AdditionalLibraryDirectories = '"$(DUILIBPATH)\\lib"';
		
		var ResTool = config.Tools('VCResourceCompilerTool');
		//resource
		ResTool.AdditionalIncludeDirectories='"$(DUILIBPATH)\\DUILIB\\include";';
		...
		CLTool.SuppressStartupBanner = true;
		CLTool.WarningLevel = warningLevelOption.warningLevel_3;
		CLTool.AdditionalIncludeDirectories = '"$(DUILIBPATH)\\DUILIB\\include";';
		if(isDllDuiEngine)
...
function GetTargetName(strName, strProjectName)
{
	try
	{
		...
		if (strName == 'dui-demo.cpp')
		    strTarget = strProjectName + '.cpp';

		if (strName == 'dui-demo.rc')
...
function AddFilesToCustomProj(proj, strProjectName, strProjectPath, InfFile)
{
	try
	{
		...
		if(isSupportNamedID)
		{//支持namedid
			cmdline= '"$(DUILIBPATH)\\tool\\residbuilder2.exe" -i "%(FullPath)" -y -p skin -r .\\duires\\winres.rc2 -n .\\duires\\name2id.xml -h .\\duires\\winres.h';
			outfiles=".\\duires\\winres.rc2;.\\duires\\winres.h;.\\duires\\name2id.xml";
		}else
		{
			cmdline= '"$(DUILIBPATH)\\tool\\residbuilder2.exe" -i "%(FullPath)" -y -p skin -r .\\duires\\winres.rc2';
